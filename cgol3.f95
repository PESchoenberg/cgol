...
contains
  ...
  subroutine cgol_config_custom(p_c1)
    ...
    do while (o1 > 0)
       ...
       call cgol_comment("1 - Neighborhood type.")
       call cgol_comment("2 - Number of iterations.")
       call cgol_comment("3 - Initial percentage of cells alive.")
       call cgol_comment("4 - Rule.")
       call cgol_comment("5 - Stop on iter.")
       call cgol_comment("6 - Wait on inter.")
       read(*,*) o1

       select case (o1)
       case (1)
          call cgol_comment("1 - Neighborhood type [1] (0 = Von Neumann, 1 = Moore)?")
          read (*,*) n1
          if ((n1.eq.0).or.(n1.eq.1)) then
             p_c1(3,1) = n1
          end if
       case (2)
          call cgol_comment("2 - Number of iterations [2000]?")
          read (*,*) n1
          if (n1.gt.0) then
             p_c1(4,1) = n1
          end if
       case (3)
          call cgol_comment("3 - Initial percentage of cells alive [75]?")
          read (*,*) n1
          if ((n1.ge.0).and.(n1.le.100)) then
             p_c1(5,1) = n1
          end if
       case (4)
          call cgol_comment("4 - Rule [75] (1 = b3s23, 2 = b3s234)?")
          read (*,*) n1
          if ((n1.ge.1).and.(n1.le.2)) then
             p_c1(6,1) = n1
          end if
       case (5)
          call cgol_comment("5 - Stop on iter [0] (0 = no, 1 = yes)?")
          read (*,*) n1
          if ((n1.ge.0).and.(n1.le.1)) then
             p_c1(7,1) = n1
          end if
       case (6)          
          call cgol_comment("6 - Wait on iter [0]?")
          read (*,*) n1
          if (n1.ge.0) then
             p_c1(8,1) = n1
          end if          
